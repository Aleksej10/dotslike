...
class Board():
    ...
    def done(self):
        if self.linije == 79228162514264337593543950335:
            return True
        if abs(self.scr) > self.points:
            return True
        return False
...
#        return 1.0*sm/n
    ...
    def playMove(self, move):
        ...
        for i in range(len(self.squares)-1, -1, -1):
            if k>1:
                break
            if (self.squares[i] & self.linije) == self.squares[i]:
                self.points -= 1
                k+=1
                if self.side == 1:
                    self.scr += 1
    ...
    def getNumpyArray(self):
        ...
        for line in self.lines:
            if (self.linije & line) > 0:
                ...
                nar.append(0)
        return array(nar).reshape(1,100)
